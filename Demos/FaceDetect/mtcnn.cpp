...
MTCNN::MTCNN(const string &model_path) {
    int ret_val = 0;
#if 0
    FILE*fp = fopen("mtcnnfp37.dat", "rb");
    ret_val += Pnet.load_param_bin(fp);
    ret_val += Pnet.load_model(fp);
    ret_val += Rnet.load_param_bin(fp);
    ret_val += Rnet.load_model(fp);
    ret_val += Onet.load_param_bin(fp);
    ret_val += Onet.load_model(fp);
    fclose(fp);
#else
    ...
    ret_val += Pnet.load_param(param_files[0].data());
    ret_val += Pnet.load_model(bin_files[0].data());
    ret_val += Rnet.load_param(param_files[1].data());
    ret_val += Rnet.load_model(bin_files[1].data());
    ret_val += Onet.load_param(param_files[2].data());
    ret_val += Onet.load_model(bin_files[2].data());
#endif

    if (0 == ret_val) init_return_ok = true;
}
...
void MTCNN::nmsTwoBoxs(vector<Bbox>& boundingBox_, vector<Bbox>& previousBox_, const float overlap_threshold, string modelname)
{
    ...
    for (auto ity = previousBox_.begin(); ity != previousBox_.end(); ity++) {
        for (auto itx = boundingBox_.begin(); itx != boundingBox_.end();) {
...
void MTCNN::extractMaxFace(vector<Bbox>& boundingBox_)
{
    ...
    for (auto itx = boundingBox_.begin() + 1; itx != boundingBox_.end();) {
        itx = boundingBox_.erase(itx);
    }
}
...
void MTCNN::ONet(){
    thirdBbox_.clear();
    for(auto it=secondBbox_.begin(); it!=secondBbox_.end();it++){
        ncnn::Mat tempIm;
        copy_cut_border(img, tempIm, (*it).y1, img_h-(*it).y2, (*it).x1, img_w-(*it).x2);
        ncnn::Mat in;
...
void MTCNN::detect(ncnn::Mat& img_, int minsize, int maxsize, std::vector<Bbox>& finalBbox_){
    ...
    if (!isSkipPNet) {
        ...
        printf("firstBbox_.size()=%lu\n", firstBbox_.size());
    }

    if (!isSkipRNet) {
        //second stage
        RNet();
        printf("secondBbox_.size()=%lu\n", secondBbox_.size());
